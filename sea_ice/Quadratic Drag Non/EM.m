...
% Rinv = [diag(1./(ones(108,1) * sigma_x).^2),zeros(108,108);zeros(108,108), diag(1./(ones(108,1) * sigma_x).^2)];
for j = 2:N-1
    ...
    FM2j = diag(alpha_l ./ m .* (exp(1i * x_loc * kk) * (uvj.* transpose(rk(1,:)))  - uj) .* abs( exp(1i * x_loc * kk) * (uvj.* transpose(rk(1,:)))  - uj));
    FM3j = diag(alpha_l ./ m .* (exp(1i * x_loc * kk) * (uvj.* transpose(rk(2,:)))  - vj) .* abs( exp(1i * x_loc * kk) * (uvj.* transpose(rk(2,:)))  - vj));
    FM1j = diag(beta_l ./ I .* (exp(1i * x_loc * kk) * ( uvj .* transpose(1i * rk(2,:) .* kk(1,:) - 1i * rk(1,:) .* kk(2,:) ) )/2 - omgj) .* abs(exp(1i * x_loc * kk) * ( uvj .* transpose( 1i * rk(2,:) .* kk(1,:) - 1i * rk(1,:) .* kk(2,:) ) )/2 - omgj));
