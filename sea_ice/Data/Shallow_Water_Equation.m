...
rng(500)
T = 44.0208; %1.03125; % total time
dt = 0.00347222; % time step
N = round(T/dt);
...
% reconstruction
Dim_Grid = 50;
[xx,yy] = meshgrid(linspace(-domain/2,domain/2,Dim_Grid), linspace(-domain/2,domain/2,Dim_Grid));
x_vec = [reshape(xx,[],1), reshape(yy,[],1)]; 
figure 
hold on
for i = 2:2:round(T/dt/100)
    u = exp(1i * x_vec * (2*pi) / domain * kk) * (u_hat(:,1+100*(i-1)) .* transpose(rk(1,:))) * domain / (2*pi);
    v = exp(1i * x_vec * (2*pi) / domain * kk) * (u_hat(:,1+100*(i-1)) .* transpose(rk(2,:))) * domain / (2*pi);
    u = reshape(u, Dim_Grid, Dim_Grid);
    v = reshape(v, Dim_Grid, Dim_Grid);
    quiver(xx, yy, u, v, 'linewidth',1)
    xlim([-domain/2, domain/2 ])
    ylim([-domain/2, domain/2 ])
    box on    
