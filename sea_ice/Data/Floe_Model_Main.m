...
x = zeros(L,N);
y = zeros(L,N);
x(:,1) = xInterpArray(1:37, 1)./100000; % Lagrangian tracers (floe center location x) evolving with time
y(:,1) = yInterpArray(1:37, 1)./100000; % Lagrangian tracers (floe center location y) evolving with time
...
for i = 2:N
    ...
    vo_x(:,i) = real(vo_x(:,i-1) + alpha_l ./ m .* (exp(1i * x_loc * kk) * (u_hat(:,i-1) .* transpose(rk(1,:)))  - vo_x(:,i-1)) .* abs(exp(1i * x_loc * kk) * (u_hat(:,i-1) .* transpose(rk(1,:)))  - vo_x(:,i-1)) * dt) + sqrt(dt) * sigma_x * randn(L,1); % velocity induced by the ocean velocity in u
    vo_y(:,i) = real(vo_y(:,i-1) + alpha_l ./ m .* (exp(1i * x_loc * kk) * (u_hat(:,i-1) .* transpose(rk(2,:)))  - vo_y(:,i-1)) .* abs(exp(1i * x_loc * kk) * (u_hat(:,i-1) .* transpose(rk(2,:)))  - vo_y(:,i-1)) * dt) + sqrt(dt) * sigma_x * randn(L,1); % velocity induced by the ocean velocity in v
