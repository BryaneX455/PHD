...
for i = 2:N
    ...
    vo_x(:,i) = vo_x(:,i-1) + alpha_l ./ m .* (exp(1i * x_loc * kk) * (u_hat(:,i-1) .* transpose(rk(1,:))) - vo_x(:,i-1)) * dt; % velocity induced by the ocean velocity in u
    vo_y(:,i) = vo_y(:,i-1) + alpha_l ./ m .* (exp(1i * x_loc * kk) * (u_hat(:,i-1) .* transpose(rk(2,:))) - vo_y(:,i-1)) * dt; % velocity induced by the ocean velocity in v
